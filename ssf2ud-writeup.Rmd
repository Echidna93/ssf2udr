...
library(rbbt) # for importing Zotero data
library(terra)
...
# update possible steps to be red
landscape[1] <- 3
landscape[2451] <- 2
landscape[50] <- 2
landscape[2] <- 2
...
land_long_possible_steps <- landscape %>% as.data.frame() %>% rownames_to_column("Var1") %>% pivot_longer(-Var1, names_to = "Var2", values_to = "value") %>% mutate(
Var1 = factor(Var1, levels = 1:ncol),
Var2 = factor(gsub("V", "", Var2), levels = 1:nrow)) %>% mutate(Var1 = as.numeric(Var1), Var2 = as.numeric(Var2))

land_possible_steps <- ggplot(land_long_possible_steps, aes(Var1, Var2,)) + geom_tile(aes(fill = value)) + 
  xlab("X") + ylab("Y") + theme(axis.text=element_blank(), axis.ticks=element_blank()) + scale_fill_gradientn(colours = c("white", "black", "red", "blue"),breaks=c(0,1,2,3)) +
    guides(fill = guide_legend(title ="Landscape type"))
ggsave("./data/output/figures/land-possible-steps.png")
...
$$
Above equation 1 gives the transition probability between the cell $c$ and a neighboring cell $c_n$. This is given by the exponential of the sum of the movement penalty $p$ and the preference of the agent $\beta$ for the neighboring cell $c_n$. The values $p$ and $theta$ were scholastically updated throughout the simulation to examine relationships between movement propensity ($p$), and preference $\theta$.

### Modelling
To retrieve the downstream selection coefficient for a given habitat type $t$ the function $fit_issf$ from the R-package AMT @signerAnimalMovementTools2019. 


![Figure 1. An image of an unsmoothed `r I(nrow)` by `r I(ncol)` domain.](./data/output/figures/land.png)

![Figure 2. An image of a `r I(nrow)` by `r I(ncol)` domain smoothed by a factor of `r I(smoothingFactor)``.](./data/output/figures/land_smooth.png)
