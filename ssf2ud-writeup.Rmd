...
getSubMatrix <- function(mat, coords, sf){
  row <- coords[1]
  col <- coords[2]
  subMatrix <- mat[(row - sf):(row+sf),(col - sf):(col + sf)]
  # top left
  subMatrix[1,1] <- NA
  subMatrix[1,2] <- NA
  subMatrix[2,1] <- NA
  # # top right
  subMatrix[nrow(subMatrix), nrow(subMatrix)] <- NA
  subMatrix[nrow(subMatrix)-1, nrow(subMatrix)] <- NA
  subMatrix[nrow(subMatrix), nrow(subMatrix)-1] <- NA
  # # bottom left
  subMatrix[1, nrow(subMatrix)] <- NA
  subMatrix[2, nrow(subMatrix)] <- NA
  subMatrix[1, nrow(subMatrix)-1] <- NA
  # # bottom right
  subMatrix[nrow(subMatrix)-1,1] <- NA
  subMatrix[nrow(subMatrix),2] <- NA
  subMatrix[nrow(subMatrix),1] <- NA
  return(subMatrix)
}
...
smoothPadMovingWindow <- function(pad, sf, nrow, ncol){
  ...
  for(i in 1:nrow(smooth)){
    for(j in 1:ncol(smooth)){
        subMatrix <- getSubMatrix(pad, c((i + sf), (j + sf)), sf)
...
j <- landscape_smooth %>%
  ...
  mutate(Var1 = factor(Var1, levels = 1:ncol),
         Var2 = factor(gsub("V", "", Var2), levels = 1:nrow))

# plot
ggplot(j, aes(Var1, Var2,)) + geom_raster(aes(fill = value))

# plot on continuous domain
out.dat$coord  <-  paste0("(", out.dat$xMod, ",", out.dat$yMod, ")")
g <- ggplot(j, aes(x = Var1, y = Var2,)) + 
