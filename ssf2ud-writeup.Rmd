...
$$
p(c_{n}) \ = \ e^{-p + \beta(c_n)} \ (1)
$$
Above equation 1 gives the transition probability between the cell $c$ and a neighboring cell $c_n$. This is given by the exponential of the sum of the movement penalty $p$ and the preference of the agent $\beta$ for the neighboring cell $c_n$. The values $p$ and $beta$ were scholastically updated throughout the simulation to examine relationships between movement propensity ($p$), and preference $\beta$.

### Modelling
To retrieve the downstream selection coefficient for a given habitat type $t$ the function $fit_issf$ from the R-package AMT (cite AMT). 
